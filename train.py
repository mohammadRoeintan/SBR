...
def main(opt):
    ...
    val_recall_history = []
    val_mrr_history = []
    train_recall_history_for_plot = [] # For plotting train recall alongside validation
    train_mrr_history_for_plot = []    # For plotting train mrr alongside validation
    ...
    if opt.validation:
        ...
        test_data_raw_for_eval = (valid_seqs, valid_targets) # Renamed to avoid confusion
        ...
        time_data_valid_for_eval = [time_data_train_orig[i] for i in indices[n_train:]] # Renamed
    else:
    ...
    # For evaluation, use test_data_raw_for_eval and time_data_valid_for_eval
    ...
    # n_node is now calculated dynamically above, so hardcoded values are removed/commented
    ...
    # else: 
    #     all_items = set()
    #     for session in train_data_raw[0] + test_data_raw[0]: # This refers to the potentially split train_data_raw
    #         all_items.update(session)
    #     # Ensure to use the original full test data for n_node calculation if needed, but dynamic calc above is better
    #     for session in test_data_raw_orig[0]: 
    #         all_items.update(session)
    #     n_node = len(all_items) + 1
    #     print(f"Calculated n_node (fallback): {n_node}")
    ...
    for epoch_num in range(opt.epoch): #
        ...
        # Train and get training metrics (avg losses)
        # The train_test function in model.py needs to be modified to return training losses
        # For now, let's assume train_test is modified to return:
        # eval_hit, eval_mrr, avg_train_total_loss, avg_train_main_loss, avg_train_ssl_loss
        # For demonstration, I'll call a modified train_test (you'll need to adjust model.py)
        # Or, we can capture losses from the prints inside train_test if modification is hard.
        # Let's assume train_test is modified to return losses.
        
        # Simulate getting training metrics (recall/mrr on training data for plotting)
        # This is usually not done every epoch as it's computationally expensive.
        # For plotting, we'll evaluate on a subset of training data or assume eval metrics are a proxy.
        # The current train_test only returns eval metrics.
        # To get training recall/mrr, we'd need to run evaluation on training data.
        
        # For simplicity, let's assume the train_test function is modified to return training losses.
        # Placeholder for actual training loss retrieval
        # avg_train_total_loss, avg_train_main_loss, avg_train_ssl_loss will be filled by train_test
        ...
        # For plotting train recall/mrr, you would ideally evaluate on the training set.
        # As a simplification here, if validation is off, we might plot test metrics as 'train'
        # or you'd need a separate evaluation pass on train_data_loader.
        # Let's assume for now we don't have separate train recall/mrr easily.
        # If you need them, add another eval pass:
        # train_hit_for_plot, train_mrr_for_plot, _, _, _ = train_test(model, train_data_loader, train_data_loader, opt, device, is_eval_on_train=True)
        # train_recall_history_for_plot.append(train_hit_for_plot)
        # train_mrr_history_for_plot.append(train_mrr_for_plot)
        ...
        if eval_hit >= best_result[0]: #
            ...
            flag = 1 #
            saved_this_epoch_path = os.path.join(model_save_dir, f'best_recall_epoch_{epoch_num}_recall_{eval_hit:.4f}_mrr_{eval_mrr:.4f}.pt') #
            state_to_save = { #
    ...
    # If you have train_recall_history_for_plot:
    # plt.plot(epochs_range, train_recall_history_for_plot, label='Train Recall@20')
    plt.title('Recall@20')
    plt.xlabel('Epoch')
    ...
    # If you have train_mrr_history_for_plot:
    # plt.plot(epochs_range, train_mrr_history_for_plot, label='Train MRR@20')
    plt.title('MRR@20')
    plt.xlabel('Epoch')
    plt.ylabel('MRR@20')
    plt.legend()
    
    # For Overfitting/Underfitting check (Example: Val Recall vs Train Loss)
    plt.subplot(2, 3, 6)
    ax1 = plt.gca()
    color = 'tab:red'
    ...
    color = 'tab:blue'
    ax2.set_ylabel('Train Total Loss', color=color) # we already handled the x-label with ax1
